...
To update the database use this command from the Web project folder (replace `TheSchlote.TaskManager` with your project's name):

```powershell
dotnet ef database update -c AppDbContext -p ../TheSchlote.TaskManager.Infrastructure/TheSchlote.TaskManager.Infrastructure.csproj -s TheSchlote.TaskManager.Web.csproj
```
